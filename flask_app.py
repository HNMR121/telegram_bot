...
def portfel(message):
    ...
    df3['Прибыльвчера'] = (df3['Ценавчера']*df3['Колво'])#+df3['суммапродажи']-df3['суммапокупки']+df3['Суммадивид']
    df = df3[df3['Колво']>0]
...
def pokazatel_4(message):
    try:
        ...
        conn.commit()
        conn.close()
        bot.send_message(message.from_user.id, 'Портфель изменен, введите Дата в формате YYYY-MM-DD:', reply_markup=markup)
...
def pokazatel_6(message):
    try:
        ...
        conn.commit()
        conn.close()
        bot.send_message(message.from_user.id, 'Тикер изменен введите количество,для дивид любое знач', reply_markup=markup)
...
def pokazatel_7(message):
    try:
        ...
        conn.commit()
        conn.close()
        bot.send_message(message.from_user.id, 'кол-во изменен введите цену,для дивид введите сумму общую', reply_markup=markup)
